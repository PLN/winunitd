...
Unknown directives are errors. `ExecStart=` must be an absolute Windows path (`SearchPath=no`). An omitted `WorkingDirectory=` is a warning; System32 is not used as a default. `Environment=` values are literals (`${}` is not expanded). A `foo.timer` activates `foo.service` when `Unit=` is omitted. `Type=scm` requires `ServiceName=` and does not use `ExecStart=`. `MemoryMax=`, `ProcessLimit=`, and `PriorityClass=` are `[Service]` only; `winctl verify` rejects bad sizes, `ProcessLimit=0` or negative, unknown `PriorityClass=` (including `realtime`), and those keys on `.timer` / `.target`.
