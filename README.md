...
Capture uses a bounded writer queue: at most 4 MiB of pending message data per invocation, 16 MiB per store, and 16,384 queued fragments. These limits exclude stream read buffers, record metadata, JSON expansion, and other manager memory. The single writer preserves enqueue order; blocked storage can affect every unit's journal, but capture keeps draining and rejects new fragments when a limit is reached. Unit status exposes cumulative `logDroppedRecords`, `logDroppedBytes` (normalized message bytes, excluding line endings), `logStorageErrors`, and `logLastStorageError`. Counters reset with the manager. Drop counters cover rejected fragments; flush/sync failures can lose additional previously buffered data. Do not treat continuation metadata as proof of a complete line after loss. Journal waits honor their context during capture and sync; close waits up to five seconds, returns an error on timeout, and retains file ownership for cleanup if storage recovers. Log reads and overall daemon shutdown still need broader lifecycle qualification.
