...
`Type=notify` stays activating until the unit writes `READY=1` on `WINUNIT_NOTIFY_PIPE` (`\\.\pipe\winunitd\notify\<unit>`), or `TimeoutStartSec` fires. `WatchdogSec=` (default `WatchdogMode=notify`) injects `WINUNIT_WATCHDOG_USEC`; a missed heartbeat fails the unit (then `Restart=`). `NotifyAccess=main` only. Scripts send `READY` / `WATCHDOG` / `STATUS` with `winunit-notify.exe`.

**Later:** WatchdogMode=tcp/http, invocation IDs, ExecStop / CTRL_BREAK / WM_CLOSE, SCM proxy, GUI attach, SessionPolicy, LoadCredential — as described in DESIGN.md.
