...
func (l *win32PipeListener) makeConnectedServerPipe() (*win32File, error) {
	...
	return l.waitForPipeConnection(p, ch)
}

func (l *win32PipeListener) waitForPipeConnection(p *win32File, ch <-chan error) (*win32File, error) {
	var err error
	select {
	case err = <-ch:
		if err != nil {
	...
	case <-l.closeCh:
		...
		p.Close()
		p = nil
		// A consumed close request must terminate the listener regardless of
		// the error racing with connection cancellation.
		<-ch
