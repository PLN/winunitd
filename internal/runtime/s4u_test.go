...
func TestS4UNames(t *testing.T) {
	t.Parallel()
	upn, realm := s4uNames(LingerRecord{Name: `TEST\ferd`})
	if upn != "ferd" || realm != "TEST" {
		t.Fatalf("DOMAIN\\user = %q %q", upn, realm)
	}
	upn, realm = s4uNames(LingerRecord{Name: "ferdinand"})
	if upn != "ferdinand" || realm != "" {
...
func TestSplitUserDomain(t *testing.T) {
	t.Parallel()
	u, d := splitUserDomain(`TEST\ferd`)
	if u != "ferd" || d != "TEST" {
		t.Fatalf("got %q %q", u, d)
	}
	u, d = splitUserDomain("ferd@example.com")
	if u != "ferd" || d != "example.com" {
		t.Fatalf("UPN = %q %q", u, d)
	}
	u, d = splitUserDomain("ferd")
	if u != "ferd" || d != "" {
