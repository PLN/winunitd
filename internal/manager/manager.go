...
func (m *Manager) ListTimers() (*protocol.ListTimersResult, error) {
	...
	for i := range out {
		...
		out[i].ScheduleState = snapshot.ScheduleState
		out[i].Activation = timerActivationStatus(snapshot.Activation)
		if snapshot.Unit != "" {
...
// Journal counters are independent observations outside the manager mutex.
