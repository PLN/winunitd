...
func testLingerHost(t *testing.T) (*UserHost, *LingerStore) {
	...
	h.cfg.Lookup = func(name string) (runtime.UserInfo, error) {
		if name == testSIDA || name == "alice" || name == `TEST\alice` {
			return runtime.UserInfo{SID: testSIDA, Username: "alice", Domain: "TEST", Profile: `C:\Users\alice`}, nil
		}
		if name == testSIDB {
			return runtime.UserInfo{SID: testSIDB, Username: "bob", Domain: "TEST"}, nil
		}
...
func TestLingerStoreRoundTrip(t *testing.T) {
	...
	rec := runtime.LingerRecord{SID: testSIDA, Name: `TEST\alice`, CredentialURI: "credman://winunitd/linger/" + testSIDA}
...
func TestStartLingeringAtBootNoSession(t *testing.T) {
	...
	if err := store.Put(runtime.LingerRecord{SID: testSIDA, Name: "alice"}); err != nil {
		t.Fatal(err)
	}
	h.StartLingering()
	if !h.Alive(testSIDA) {
...
`,
	...
	_, err := m.Handle(context.Background(), protocol.MethodEnableLinger, json.RawMessage(`{"user":"alice"}`))
	if err == nil {
