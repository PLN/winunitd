...
func TestParseSince(t *testing.T) {
	...
	tests := []struct {
		...
		{in: "30min", want: now.Add(-30 * time.Minute)},
		{in: "1h 30min", want: now.Add(-90 * time.Minute)},
		{in: "1 day 2 hours ago", want: now.Add(-26 * time.Hour)},
		{in: "1d ago", want: now.Add(-24 * time.Hour)},
		{in: "", wantErr: true},
