...
func TestAllMethodsRoundTrip(t *testing.T) {
	...
	h := HandlerFunc(func(ctx context.Context, method string, params json.RawMessage) (any, error) {
		...
		case MethodOperation, MethodCancelOperation:
			...
			return OperationResult{ID: p.ID, State: "succeeded"}, nil
		case MethodVerify:
			var p VerifyParams
			if err := DecodeParams(params, &p); err != nil {
				return nil, err
	...
	if _, err := client.EnableLinger(ctx, "alice"); err != nil {
		t.Fatal(err)
	}
	if _, err := client.DisableLinger(ctx, "alice"); err != nil {
		t.Fatal(err)
	}
...
func TestNonAdminEnableLingerFailsClosed(t *testing.T) {
	...
	defer stop2()
	_, err = client2.EnableLinger(context.Background(), "alice")
	pe, ok = err.(*Error)
	if !ok || pe.Code != CodePermissionDenied {
...
func TestAdminEnableLingerAllowed(t *testing.T) {
	...
	defer stopSys()
	if _, err := clientSys.EnableLinger(context.Background(), "alice"); err != nil {
