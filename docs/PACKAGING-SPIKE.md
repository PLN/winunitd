...
The same hash-verified MSI pair also passed the strict helper lane as SYSTEM on Windows 11 Enterprise LTSC Evaluation **26100.9168**, offline, on September 5, 2026. Install and repair returned 0; both running/stopped rollback cases returned the expected 1603 and restored their asserted state. The stop-deadline rollback returned 1603 after **201.37 seconds**; the slow upgrade returned 0 after **48.69 seconds**; uninstall returned 0 and left no fixture service or executable. Evidence was collected before media removal and normal shutdown. The guest's repeatedly offered Windows Security app update remains a [baseline acceptance limitation](TEST-LAB.md#maintained-baseline-observations--september-5-2026); these observations do not establish general desktop patch compliance or production installer qualification.
