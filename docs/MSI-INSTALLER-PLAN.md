...
4. **Bounded maintenance shutdown.** Stop accepting new starts, disarm triggers, cancel restart loops, stop user managers and system units, and wait for owned processes/file handles to close. Apply one aggregate deadline, not an unbounded sum of per-unit deadlines. `stopUnitCtx` currently ignores its context and `UserHost.Close` kills user managers directly; these need an explicit reviewed maintenance contract. Application shutdown is currently forceful; the first alpha MSI must say so. Graceful ExecStop/CTRL_BREAK support is a separate runtime feature, not an installer promise.
...
| --- | --- | --- |
| M0: packaging spike | Pin WiX/SDK; record terms; choose helper implementation; prototype advanced service settings, long-stop, and rollback in a VM | Query-back parity with current SCM settings; injected failure restores the old service |
| M1: runtime readiness | User-manager recovery, truthful service readiness, diagnostics, bounded maintenance, version metadata, isolated integration tests | Elevated and LocalSystem tests pass; no retry during stop/upgrade |
| M2: first MSI | `packaging/wix/` project with files, ACLs, PATH, service/event source, small UI, quiet install, uninstall | Fresh offline install/repair/uninstall in a clean VM; user state retained |
| M3: servicing and migration | Major upgrades, rollback helpers, conflict detection, pilot migration with dry-run and rollback | N-1 to N and failed-upgrade tests with running system/user workloads |
| M4: release pipeline | Public-release preparation; SignPath evaluation/enrollment or fallback provider; pinned packaging CI, Authenticode signing, attestations, checksums, notices, immutable GitHub release | Provider accepted and integrated; installed signatures, provenance, and hashes verified; clean-machine acceptance matrix passes |
| M5: pilot replacement | Migrate dev from Task Scheduler to MSI/SCM ownership | Real reboot, logon/logoff, manager crash, repair, upgrade, and rollback exercised |
...
Sign and timestamp executable/helper payloads **before** packaging, then sign and timestamp the MSI, and verify both embedded payloads and the package. Select a signing identity/provider during M4 and protect its credentials in a release-only environment. An unsigned artifact is an internal test candidate, not the public installation release. Document an offline-verification policy separately from online revocation/timestamp checks. See [Microsoft Authenticode timestamping](https://learn.microsoft.com/en-us/windows/win32/seccrypto/time-stamping-authenticode-signatures).
...
The first deliverable is a VM-tested internal MSI candidate after M0-M2. A public installation release requires M3-M4; replacing dev's working pilot is M5.
