...
The product package source is [packaging/wix](../packaging/wix). It is a different product from the unsigned beta in [packaging/beta](../packaging/beta): separate UpgradeCode, binaries under `winunitd\bin`, an owned PATH component, and MSI event-source registration. The product detects the beta UpgradeCode and refuses to install beside it. It does not call `winunitd install` or `uninstall`. Operator steps are in [INSTALLATION.md](INSTALLATION.md). Quiet install, repair, and uninstall passed as SYSTEM on one guest (`9961798-r64-accept`). For 0.1-alpha, R6.1 is checked: those rows are recorded, and `gui-install` passed on an interactive Eval desktop under `7d21de2-r64-finish`. Claimed Windows 11 Enterprise and Windows 11 Enterprise LTSC GUI stays deferred-media. The authored service recovery reset is 49710 days because the WiX utility extension cannot express the infinite reset used by manual registration. The product package now authors the R6.2 quiesce and 180-second stop before file replacement, and restores captured service configuration on rollback. Native SYSTEM evidence for that transaction is recorded as `b915fbd-r62-servicing` (raw logs private). R6.3 keeps mutable machine and user data outside file components and rejects conflicting registrations, custom base directories, and unsafe directories before service start. Native SYSTEM `preflight-reparse` and `preflight-unmanaged-service` on one guest are recorded as `9961798-r64-accept`. The repeatable acceptance harness is `tests/installer`. For 0.1-alpha, R6.1 and R6.4 are checked from the runnable set in [R6 evidence](R6-EVIDENCE.md#7d21de2-r64-finish). deferred-media and deferred-older-msi were not run. The R6.5 command and VM fixture are in `tools/migrate`. Native fixture evidence `4223ac9-r65-migrate` is verified on tip `f4e6c477ada211cf5df736a16ea7685a612412a4`, so R6.5 is checked. `-execute-msi` stays unqualified. For 0.1-alpha only, overall R6 is checked under the [#251](https://github.com/PLN/winunitd/issues/251) deferral policy. The documented limitations are deferred-media and deferred-older-msi. R4 acceptance (R4.1, R4.4, and R4.5) remains an unmet prerequisite for the fully qualified release. This alpha check does not itself authorize the R7 handoff. R7, the live Hermes handoff, has not started. Installer version `0.1.0` is the only recorded product package, so an N-1 package has to be an earlier recorded installer version with this UpgradeCode.
...
Initial qualification targets: Windows 11 Enterprise and Enterprise LTSC x64, and Windows Server 2022/2025 x64, including Server Core. The maintainer selected Enterprise/LTSC for the first desktop release; Pro is outside this initial qualification matrix. These are proposed test targets, not a claim of existing qualification. Add ARM64 only with its own native package and test coverage. No x86 or dual per-user/per-machine MSI in the first release. Installed binaries must run without Go, WiX, .NET, Python, or a package download on the destination machine.
...
The runtime has advanced beyond the original packaging review. Remaining qualification and installer integration must use these implemented contracts:

1. **User-manager crash recovery.** Bounded recovery, fresh-token acquisition, stale-result rejection and cancellation are implemented. Genuine SYSTEM/interactive and headless-user recovery evidence is recorded in [R2 evidence](R2-EVIDENCE.md); the full multi-user/session/security matrix remains open under R4. Installer maintenance must retain authority over recovery throughout file replacement and rollback. This is the service path that replaces the pilot task's recovery role.
2. **Truthful startup readiness.** SCM now remains `StartPending` until listener/coordinator readiness. Control stays available when initial workload configuration is rejected. Runtime readiness is separate from workload health. R6 still requires a bounded installer health check that verifies server identity and expected binary/protocol version, plus startup and rollback failure qualification.
...
4. **Bounded maintenance shutdown.** The [global maintenance barrier](OPERATIONS.md#global-maintenance), tracked ExecStop helpers, aggregate deadlines and retained cleanup failures are implemented. [Native cooperative/forced-stop evidence](R3-EVIDENCE.md#tracked-cooperative-stop) covers SYSTEM and headless users. R6 must integrate quiescence, confirmed SCM process exit, replacement and rollback into one installer transaction; the beta's stopped-service preflight is not that transaction. Unspecialized workloads may still require forced termination; console/GUI signals remain deferred.
5. **Reliable version/build identity.** Build-injected release metadata and source/artifact manifests are implemented. PE `VERSIONINFO` resources for every executable remain required from the same release identity. Keep the control-protocol version separate. Complete service-path quoting and non-ASCII account/profile qualification.
6. **Test isolation.** Dedicated test endpoints and guards are implemented under [R0.2](MILESTONES.md). Preserve those guards and run installer acceptance in disposable VMs with no developer pilot.
...
The [packaging spike](PACKAGING-SPIKE.md) proves a fixture rollback-aware long-stop action. Microsoft's ServiceControl documentation describes a 30-second wait, while the checkpoint-reporting fixture completed a 45-second native stop on the tested guest. Neither establishes a portable guarantee for winunitd's intended 180-second budget. The product package embeds that helper: quiesce, then confirm SCM stop and process exit for 180 seconds, before `StopServices` and file replacement. `RemoveExistingProducts` is scheduled after `InstallExecute`. An early removal cannot have script actions between `InstallInitialize` and itself (ICE63, MSI error 2613). `ServiceControl` remains and runs against an already stopped service. `StopServices` and `DeleteServices` are skipped when a newer package is removing this product, so that removal does not stop the service the new package just started. A timeout or a payload file that is still open aborts the transaction rather than replacing a live executable. See [Microsoft ServiceControl table](https://learn.microsoft.com/en-us/windows/win32/msi/servicecontrol-table) and [installation servicing](INSTALLATION.md#servicing-a-running-manager). Native SYSTEM qualification of the product transaction is still required.
...
Use major upgrades initially, block downgrades, and schedule removal of the old product after `InstallExecute` so it participates in rollback and the servicing helper can stop the service before any file replacement. Do not place actions between `InstallInitialize` and `RemoveExistingProducts`; that sequence is ICE63 and fails at runtime with MSI error 2613. Keep same-version replacement disabled; reinstalling the same published MSI is a maintenance/repair operation. Do not start old and new managers concurrently. See [WiX major-upgrade scheduling](https://docs.firegiant.com/wix/schema/wxs/majorupgrade/).
...
R6.5 is the migration command in [tools/migrate](../tools/migrate/README.md) and the disposable VM pilot fixture in `tools/migrate/fixture`. The command discovers, backs up, dry-runs, fail-closes on conflicts, hands off ownership, checks health, and rolls back. It is not an MSI custom action and it does not call `winunitd install` or `winunitd uninstall`. The caller supplies the MSI path. The fixture uses alice, bob, and carol only. It mimics a Task Scheduler interactive pilot and a unit/enable tree. It does not install Hermes. Native fixture evidence `4223ac9-r65-migrate` records that script on tip `f4e6c477ada211cf5df736a16ea7685a612412a4`. `-execute-msi` stays unqualified. R7 is the live Hermes handoff and soak. R7 has not started.

The fixture exercises export, disable, stop, copy, journal archive, conflict fail-closed, health, and inverse restore. It does not retain a Hermes home, preserve WebUI behavior, or check live component health. Those checks stay in R7. For the live dev machine, R7 will:
...
| R6.1 / R6.4 (former M2) | Package project, version metadata, files, ACLs, PATH, service/event source, quiet install, repair, uninstall | For 0.1-alpha, the runnable set passed under `9961798-r64-accept` and `7d21de2-r64-finish`. deferred-media (claimed SKUs) and deferred-older-msi were not run. The harness is `tests/installer` |
| R6.2–R6.5 (former M3) | Major upgrades, rollback, conflicts, and the explicit migration command plus VM pilot fixture | R6.2 servicing evidence is recorded. The R6.5 command and fixture are in `tools/migrate`. Native fixture id `4223ac9-r65-migrate` is verified on tip `f4e6c477ada211cf5df736a16ea7685a612412a4`. deferred-older-msi N-1 was not run. R7 has not started |
...
- Conflicting manual/pilot installs, hostile/reparse-point data directories, and the R6.5 fixture migration/rollback. Live Hermes authentication and component health checks stay in R7.
