...
- [ ] **R2.2 Coordinator:** timer and session/admission/linger policy now use serialized decision handlers. Finish the complete [writer audit](LIFECYCLE-WRITERS.md), including I/O boundaries and completion authority, using the Design section 2 contract. Preserve independent blocking I/O and prohibit transaction-result replay. [Policy acceptance](R2-EVIDENCE.md#user-policy-and-cleanup-admission-acceptance).
- [ ] **R2.3 Scheduling:** ordinary/diagnostic/stop overload and completion progress passed combined control tests; reserved maintenance quiesced real workloads with all 128 ordinary connections occupied. Native linger revocation has reserved admission, and retired-SID/reload cleanup requests are bounded and coalesced. [Control acceptance](R2-EVIDENCE.md#control-overload-and-reserved-maintenance-acceptance), [native-work acceptance](R2-EVIDENCE.md#user-policy-and-cleanup-admission-acceptance). Remaining: complete the full worker-class audit. Handler extraction alone does not satisfy this gate.
