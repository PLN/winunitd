...
- [x] **R6.3 Data/compatibility:** repair, upgrade, and uninstall keep machine and user units, journals, timer state, and linger records. Those files stay outside MSI file components, so repair does not overwrite units, reset enable links, or recreate user configuration. There is no purge option. Format changes require an explicit migration; rollback-compatible readers stay, and MSI rollback does not undo an application-data migration. Before service start or ownership changes, the package rejects an unmanaged or unrelated `winunitd` service, a custom base directory, a machine-wide pilot launcher, and a data directory with unexpected ownership or a reparse point. It does not follow the link to repair ACLs. This check is the source and test slice for [#243](https://github.com/PLN/winunitd/issues/243). Native SYSTEM qualification of that preflight is not claimed. The harness cases `preflight-reparse` and `preflight-unmanaged-service` are still unrecorded; see [R6 evidence](R6-EVIDENCE.md#acceptance). R6.4, R6.5, and overall R6 stay open.
- [ ] **R6.4 Acceptance:** execute clean GUI/quiet/SYSTEM/offline install, repair, uninstall, reinstall, downgrade rejection, N-1 upgrade, locked-file/reboot-required, and rollback matrix on every claimed platform. The harness and the unrecorded case list are in [R6 evidence](R6-EVIDENCE.md#acceptance) ([#245](https://github.com/PLN/winunitd/issues/245)). No acceptance evidence id is recorded. Remaining claimed SKUs: Windows 11 Enterprise x64, Windows 11 Enterprise LTSC x64, Windows Server 2022 x64, Windows Server 2025 x64, and Windows Server Core x64. Historical servicing id `b915fbd-r62-servicing` stays outside this box. One filled SKU would still leave the others required. R6.5 and overall R6 stay open. A3 / R4.4 stay deferred.
