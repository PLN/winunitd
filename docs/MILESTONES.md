...
- [ ] **R3.3b Services/stop:** single ExecStop implemented with captured invocation context, separate bounded helper ownership/output, shared graceful/forced budget, late-launch retention and cleanup retries. Parser and manager race tests cover completion, hangs, failed/late cleanup, failed startup, oneshots and capacity. Real Windows identity/job/failure qualification remains open. Console/GUI signals remain deferred.
