...
- [ ] **R1.2 Activation:** process creation returns before oneshot completion; output capture attaches before waiting for exit. Required Windows tests verify all 5,000 stdout/stderr lines, no-newline fragments, startup timeout cleanup, and explicit stop while waiting. Existing completed-oneshot active state is preserved until R3. Broader failure qualification remains coupled to R1.3.
- [ ] **R1.3 Stop results:** explicit stop, failed-state reaping, ordinary exit, replacement of an exited invocation, readiness/watchdog failure, and late launch disposal retain process ownership until cleanup succeeds. Failures remain available for stop retry and block replacement/restart. Retries join a pending adapter call after caller timeout. The Windows adapter verifies an empty Job Object process list before closing handles and preserves handles on kill/wait/query failure. Shutdown closes start admission before its stop snapshot and includes accepted launches even before they publish a process. Shutdown context deadlines now bound operation-lock, pending process-stop, and journal waits; expiration retains pending ownership and permits shutdown retry. SCM/task stop retries also join a pending native call after deadline instead of spawning another blocked call. Watch closure, daemon/user-manager integration, and remaining asynchronous teardown paths still need qualification. Stop remains forced Job Object termination until R3.
...
Evidence: the bounded journal implementation passed every GitHub CI lane at `ee64959`; subsequent ownership/trigger/pending-stop changes passed through `fe099e9`. Later full local race suites and vet pass, including real child-exit confirmation and controlled late-launch stop/close/shutdown interleavings. CI exposed fake-clock tests advancing cleanup deadlines instead of the intended readiness/activation deadline. Readiness now uses a distinct duration; the repeating-timer test waits for first-invocation cleanup before advancing and passed 100 focused repetitions. Controlled shutdown deadline tests cover blocked launch/stop, native SCM/task calls, and hung capture; ten repetitions, the full race suite with CI flags, and vet pass. Latest CI confirmation remains pending. These results are implementation progress, not milestone closure or installation qualification.
