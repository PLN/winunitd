...
- [ ] **R0.4 Windows harness:** implement the [qualification lab plan](TEST-LAB.md) for disposable VM setup and evidence collection for SCM, SYSTEM, standard users, sessions, reboot, and installer failure injection. Fresh Server 2025 Core and Windows 11 Enterprise LTSC evaluation guests passed controller-driven SYSTEM/SCM/reboot smoke using exact CI artifacts. Isolated networking, scoped API access, ownership records, and artifact admission are implemented. Automated media preparation, maintained baselines, cleanup/reconciliation, CI integration, and broader scenarios remain pending.
