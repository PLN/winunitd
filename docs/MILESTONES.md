...
- [ ] **R5.4 Diagnostics/API:** provide durable daemon diagnostics and Windows event resources; expose operation/invocation/configuration identity, lifecycle/health/load state, last errors, restart budget, and timer/storage state in bounded responses. R5.4a is the runtime slice: restart budget on status and snapshot, timer/storage on status and list-timers, and a root-bounded daemon log. It does not close R5.4. R5.4b authors the Application event source, message resources, and startup-failure emission on that same daemon-log path. Rendered-event evidence on a clean Windows install is still required. This does not waive A3 / R4.4.
