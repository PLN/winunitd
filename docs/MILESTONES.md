...
- [ ] **R0.4 Windows harness:** implement the [qualification lab plan](TEST-LAB.md) for disposable VM setup and evidence collection for SCM, SYSTEM, standard users, sessions, reboot, and installer failure injection. Access/inventory discovery is recorded; baselines, controller, and smoke remain pending. Establish a baseline smoke run with its actual identity recorded.
