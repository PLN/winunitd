...
- [ ] **R1.4 Capture bounds:** stdout/stderr capture now emits UTF-8 fragments of at most 64 KiB before newline/EOF, with continuation/partial metadata in journal v3 and the logs API. Mixed v1/v2 readers remain covered; fragment reconstruction, Unicode boundaries, early emission, and stored metadata have regressions. Buffering across streams/invocations, slow/full-storage isolation, and dropped-output/error reporting remain open; file writes are still synchronous.
