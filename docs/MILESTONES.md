...
- [ ] **R1.4 Capture bounds:** queue policy qualified at `f5f4e37` by exact-source Windows/Linux race CI, focused pressure/accounting tests and simultaneous real-child producers under a storage stall. Remaining to close: combine measured memory/worker bounds and status/stop responsiveness with aggregate output/storage pressure, as required by [issue #95](https://github.com/PLN/winunitd/issues/95). Overall lifecycle admission remains R2.3; journal file/retention bounds remain R5.3. [Evidence and limits](R1-EVIDENCE.md#september-13-aggregate-capture-fairness).
