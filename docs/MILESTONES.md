...
- [ ] **R3.4 Readiness/liveness:** separate startup readiness from health/recovery; provide explicit probe/grace/threshold policies and bounded restart backoff. Format-2 capped exponential restart backoff is implemented; [contract](UNIT-REFERENCE.md#recovery-delay-and-backoff). Endpoint readiness and probe grace/timeout/threshold policies are implemented; native combined acceptance remains. Tag results with invocation identity; preserve rate limits for timer/watch activations.
