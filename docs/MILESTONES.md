...
- [ ] **R2.1 Records/events:** bounded immutable snapshots include units, active operations and accepted system user-host ownership, with host-scoped instance IDs and policy revisions. Native liveness remains a separate observation. Complete coordinator-wide identity-bearing event coverage. [Native snapshot evidence](R2-EVIDENCE.md#system-user-host-snapshot-qualification), [snapshot contract](OPERATIONS.md).
- [ ] **R2.2 Coordinator:** timer and session/admission/linger policy now use serialized decision handlers; transition diagnostics and adapter error observations preserve independent progress. Finish the complete [writer audit](LIFECYCLE-WRITERS.md), including I/O boundaries and completion authority, using the Design section 2 contract. Preserve independent blocking I/O and prohibit transaction-result replay. [Policy acceptance](R2-EVIDENCE.md#user-policy-and-cleanup-admission-acceptance), [diagnostic acceptance](R2-EVIDENCE.md#nonblocking-lifecycle-diagnostics-and-error-observations).
- [ ] **R2.3 Scheduling:** ordinary/diagnostic/stop overload and completion progress passed combined control tests; reserved maintenance quiesced real workloads with all 128 ordinary connections occupied. Native linger revocation has reserved admission, retired-SID/reload cleanup requests are bounded and coalesced, and canceled recovery releases obsolete gate waits. [Control acceptance](R2-EVIDENCE.md#control-overload-and-reserved-maintenance-acceptance), [native-work acceptance](R2-EVIDENCE.md#user-policy-and-cleanup-admission-acceptance), [recovery acceptance](R2-EVIDENCE.md#canceled-recovery-worker-acceptance). Remaining: complete the full worker-class audit. Handler extraction alone does not satisfy this gate.
...
Status: technical packages delivered; milestone acceptance depends on R2. Outcome: an explicit v2 behavior contract and controlled migration from alpha semantics.
