...
| lifecycle_bound.go: queueBoundStopsLocked, runBoundStops, stopBoundMember | Captured dependency-stop scope, coalesced pending records, reserved worker, operation identity and current-owner checks | Managed exit/failure acceptance queues cleanup under m.mu. One worker executes captured plans outside the decision lock; late work cannot change replacement process or operation metadata. External SCM/task disappearance observation remains open |
