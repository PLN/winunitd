...
| lifecycle_events.go and lifecycle_launch.go: admission and completion handlers | Member state/error, matching generation, cleanup ownership, restart decision | Typed boundary exists; route through the coordinator after remaining workers stop writing records |
| supervise.go: launchUnitOwnedOp | Invocation generation/configuration, process adoption, notify ownership, readiness/oneshot results, start cancellation, late cleanup | Launch admission, previous cleanup, invocation metadata and partial-failure adoption now use captured effects/handlers. Successful process adoption and readiness/oneshot completion remain to migrate |
